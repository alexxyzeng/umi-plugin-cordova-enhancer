...
var commands = {
  ...
  releaseAndroid: function releaseAndroid() {
    failure({
      message: "当前功能未实现"
    });
  }
...
function getCordovaDetails(_, success, failure) {
  ...
  var installedPlatforms = platforms.slice(0, platforms.indexOf("Available platforms")).replace("Installed platforms:", "").split("\n");
  var plugins = _child_process.default.execSync("cordova plugin ls") + "";
  success && success({
    data: {
